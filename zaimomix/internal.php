...
switch($this->uri->segment(1))
{
	case 'about': 
		...
		$description = 'Zaimomix осуществляет посреднические услуги между клиентом, который хочет получить деньги в заём, и кредитным учреждением, чья деятельность лицензирована';
		break;
	case 'faq': 
	...
	case 'soglasie': 
		...
		$description = 'Условия получения мгновенных займов и кредитов с помощью лучшего онлайн сервиса  Zaimomix с самой низкой процентной ставкой';
		break;
	case 'rules': 
		$my_title = 'Основные Правила для Предоставления Срочных Займов'; 
		$description = 'Онлайн сервис  Zaimomix предоставления срочных займов и кредитов с самой низкой процентной ставкой предоставляет условия для получения займов';
        break;
    case 'allarticles': 
	...
	case 'asked-questions':
		$my_title = 'Преимущества срочных займов'; 
		$description = 'Без сомнения, личные займы сегодня – это удобный и оперативный вид кредитования граждан. Микрозайм представляет собой небольшой по сумме заем на относительно короткий срок. Последний составляет обычно от нескольких недель до нескольких месяцев.';
		break;
	case 'microfinance':
		$my_title = 'Исторические особенности микрофинансирования'; 
		$description = 'Точкой отсчёта истории микрозаймов принято считать 1976 г. В это время тогда ещё не известный профессор Мухаммед Юнус принялся выдавать срочные займы на малые суммы нуждающимся гражданам Бангладеша. Интересен тот факт, что микрозаймы выдавались с определённым условием: клиенты обязаны были открыть собственное дело или же развить семейный бизнес.';
		break;
	case 'bad-credit-history':
		$my_title = 'Займ при плохой кредитной истории: возможно ли?'; 
		$description = 'Негативную кредитную историю не нужно считать приговором и неисправимой ситуацией. Действительно, в подобном случае взять долгосрочный займ в банковском учреждении довольно сложно. Поскольку заметно высока вероятность отказа по ипотечным кредитам и продуктам с низкой процентной ставкой.';
		break;

	default: $my_title = 'Срочные Займы Круглосуточно без Проверок Онлайн'; $description = 'Zaimomix - лучший онлайн сервис по выдаче мгновенных займов и кредитов без проверки вашей кредитной истории.Только у нас лучшие кредитные предложения!'; break;
} 
	...
	elseif ($this->uri->segment(1) == 'asked-questions') require 'internal-asked-questions.php';
	elseif ($this->uri->segment(1) == 'microfinance') require 'internal-microfinance.php';
	elseif ($this->uri->segment(1) == 'bad-credit-history') require 'internal-bad-credit-history.php';
