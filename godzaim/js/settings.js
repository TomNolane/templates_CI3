...
$("#email").suggestions({ 
  ...
  $('#email').on('keyup keypress', function(e) {
      if($(this).val().match(/([а-яёА-ЯЁ]+)/)){
          ...
          if(lang==1){
              $(this).parent().addClass('has-error');
              $(this).after('<span class="help-block form-error help-lang" style="margin-top: 15px !important;">Пожалуйста, смените раскладку клавиатуры на <span class="label label-info">EN</span></span>');
              $(this).parent().prev().removeClass('label_er').addClass('label_true');
          }
      } else {
        lang=0;
