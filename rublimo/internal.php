...
else
{
	...
	switch($this->uri->segment(1))
	{
		...
		case 'news6':
			...
			break;
		case 'news7':
			$my_title = '«Эх, дал бы кто взаймы»: Как получить займ на Qiwi-кошелёк без отказа'; 
			$description = 'Предположим, у вас чрезвычайная ситуация в жизни: надо купить медикаменты, ребёнка срочно собрать в школу, подарить супруге ирисы небесно-синего цвета на годовщину или заказать на «АлиЭксперсс» по ограниченной акции шикарнейший гироскутер (безусловно, перспективное вложение).';
			break;
		case 'news8':
			$my_title = '«Есть Контакт!»: Как получить заём через CONTACT без лишней головной боли'; 
			$description = 'Ситуации, когда деньги нужны позарез, бывали, пожалуй, у каждого из нас. Это может быть счёт за ремонт прорванных труб, необходимость поставить зимнюю резину – да хоть прикупить очки Ray Ban со стразами для коррекции зрения. Причин может быть множество.';
			break;
