$('#agree').change(function () {
    if ($('#agree:checked').val() == '1') $('#next').css('display', 'block');
...
$(document).ready(function () {
    ...
    $('#email').on('keyup keypress', function (e) {
        if ($(this).val().match(/([а-яёА-ЯЁ]+)/)) {
            ...
            if (lang == 1) {
                $(this).parent().addClass('has-error');
                $(this).after('<span class="help-block form-error help-lang">Пожалуйста, смените раскладку клавиатуры на <span class="label label-info">EN</span></span>');
            }
        } else {
            lang = 0;
            $(this).parent().removeClass('has-error');
